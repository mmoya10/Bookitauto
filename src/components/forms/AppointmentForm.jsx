...
useEffect(() => {
  ...
  const extrasOpts = useMemo(() => {
    ...
    return calendars
      ...
      .map((e) => ({ id: e.id, label: e.name, duration: e.duration, price: e.price }));
...
const totalPrice = useMemo(() => {
  ...
  const slotOptions = useMemo(() => {
    const baseOptions = slots.length
      ? slots.map((s, i) => ({ value: String(i), label: formatSlotLabel(s.start, s.end) }))
      : [{ value: "-1", label: "Sin huecos" }];

    if (isEdit) {
      const currentLabel = formatSlotLabel(new Date(start), new Date(end));
      return [{ value: "current", label: `Hora actual: ${currentLabel}` }, ...baseOptions];
    }
    return baseOptions;
  }, [slots, isEdit, start, end]);

  const slotValue = isEdit && selectedSlotIdx === -1 ? "current" : String(selectedSlotIdx);
...
if (Array.isArray(selectedCal?.options) && selectedCal.options.length > 0 && !optionId) {
  ...
  return;
    ...
    >
        ...
        {!creatingUser ? (
          <div className="flex gap-2">
            <Select
              ...
              className="flex-1"
            />
            <Button type="button" onClick={() => setCreatingUser(true)}>+</Button>
          </div>
        ) : (
      ...
      <div className="grid gap-1.5">
        ...
        />
        <div className="text-[11px] text-slate-400">Duración total: {totalDuration} min</div>
      </div>
  ...
  <div className="grid gap-1.5">
    ...
    <Select
      options={optionOpts.map((o) => ({
        ...
        label: `${o.label} ${o.duration ? `· +${o.duration} min` : ""} ${o.price ? `· +${Number(o.price).toFixed(2)} €` : ""}`.trim(),
      }))}
      value={optionId || ""}
    ...
    {optionId && (() => {
      ...
      return (
        <div className="mt-2 flex gap-3 items-start p-2 rounded-xl border border-white/10 bg-white/5">
          <img
            src={sel.imageUrl || "https://placehold.co/120x90?text=Opción"}
            alt=""
            className="w-24 h-20 object-cover rounded-lg border border-white/10"
          />
          <div className="min-w-0">
            <div className="text-sm font-semibold">{sel.label}</div>
            <div className="text-xs text-slate-300">{sel.description || "—"}</div>
            <div className="mt-1 text-xs text-slate-300">
              {sel.duration ? `+${sel.duration} min` : ""}
              {sel.price ? `${sel.duration ? " · " : ""}+${Number(sel.price).toFixed(2)} €` : ""}
            </div>
          </div>
        </div>
      );
    ...
    <div className="text-[11px] text-slate-400">
      {optionId ? "Has seleccionado una opción." : "Este servicio requiere una opción."}
    </div>
  </div>
)}
        ...
        <div className="grid gap-1.5">
          ...
          <Select
            ...
            onChange={(val) => {
              ...
              const s = slots[i];
              if (s) { setStart(toLocalInput(s.start)); setEnd(toLocalInput(s.end)); }
            }}
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        {isEdit && <Button type="button" onClick={onOpenComplete}>Completar cita</Button>}
      </div>
    </form>
  );
}
