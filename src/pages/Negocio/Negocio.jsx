...
const glass =
  "rounded-2xl border border-white/10 bg-white/10 backdrop-blur-xl shadow-[0_10px_30px_rgba(0,0,0,0.35)]";
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <section
        className={clsx(
          glass,
          "p-5 grid grid-cols-1 gap-4 md:grid-cols-[220px_1fr]"
        )}
      >
        {/* Logo */}
...
function BranchModal({ initial, onClose }) {
  ...
  const mSave = useMutation({
    mutationFn: (payload) =>
      isEdit
        ? updateBranch({ id: initial.id, ...payload })
  ...
  return (
    <Portal>
      <div
        className="fixed inset-0 grid p-4 bg-black/40 place-items-center"
        onClick={onClose}
      >
        <div
          className={clsx(glass, "w-[min(600px,95vw)] p-4")}
        ...
        >
          ...
          <form onSubmit={submit} className="grid gap-2">
            {["nombre", "direccion", "cp", "ciudad", "email", "telefono"].map(
              (f) => (
