...
export default function StockPage() {
  ...
  // Movimientos: siempre por mes; si hay producto → filtra por productId, si no → todos
  const { data: movements } = useQuery({
    ...
    queryFn: () =>
      fetchMovements({
        ...
        productId: selectedId || undefined, // si no hay producto -> todos
      }),
  });
    ...
    <div className="space-y-5 text-zinc-100">
      <header>
        ...
        <p className="text-sm text-slate-300">
          Control de alertas, consulta de movimientos y actualización de
          existencias.
        </p>
      </header>
      ...
      <section className={clsx(glassCard, "p-4")}>
        <div className="mb-2">
          ...
          <p className="text-xs text-slate-300">
            Productos por debajo del mínimo de aviso.
          </p>
        </div>
        ...
        {lowStock && lowStock.length ? (
          <HScroll>
            {lowStock.map((p) => (
              <div
                key={p.id}
                className="min-w-[240px] rounded-xl border border-red-400/30 bg-red-500/10 p-3"
              >
  ...
  <div>
    <h2 className="text-base font-semibold">Movimientos del mes</h2>
    <p className="text-xs text-slate-300">
      {selectedId
        ? `Producto seleccionado · ${monthLabel}`
        : `${monthLabel} · todos los productos`}
    </p>
  </div>
...
function MovementForm({ movement, products, onSubmit, submitting }) {
  ...
  useEffect(() => {
    if (form.reason === "venta" && form.kind !== "gasto")
      setForm((f) => ({ ...f, kind: "gasto" }));
    if (form.reason === "compra" && form.kind !== "ingreso")
      setForm((f) => ({ ...f, kind: "ingreso" }));
